...
Degistirmen gereken TEK yer: asagidaki HISSELER listesi.
"""
...
# ============================================================================
#  TARANACAK HISSELER  —  BURAYI DEGISTIR
#  Kodlari alt alta ya da yan yana yazabilirsin, virgul de olur.
#  Kendi tarayicindaki listeyi buraya yapistirman yeterli.
# ============================================================================
...
WEDGE = dict(min_gap=5, min_width=20, max_width=200, lookback=100,
             convergence=0.75, max_wait=50, trend_min=0.5, min_quality=55)
...
TRI = dict(order=4, min_piv=3, window=90, min_bars=22, sapma_max=1.8,
           flat=0.0006, converge=0.80, min_quality=55, max_age=14)
...
DBL = dict(order=6, max_diff=0.018, min_sep=12, max_sep=55, min_depth_atr=4.0,
           min_depth_pct=0.055, trend_min=1.6, min_quality=62, max_age=12,
           uc_pencere=40)
...
ISCI = int(os.environ.get("ISCI", "8"))
MIN_KALITE = int(os.environ.get("MIN_KALITE", "72"))
...
def mumlari_cek(kod):
    """Yahoo'dan 1 yillik gunluk OHLC. Iki host da denenir."""
    for host in HOSTS:
...
def bir_hisse(kod):
    try:
        veri = mumlari_cek(kod)
        if veri is None:
            return kod, None, "veri yok"
        high, low, close, times = veri
        r = tara({"high": high, "low": low, "close": close}, times=times)
        if not r or r.get("kalite", 0) < MIN_KALITE:
...
def main():
    ...
    if not kodlar:
        ...
        sys.exit(1)
    print(f"{len(kodlar)} hisse taraniyor ({ISCI} es zamanli istek)…")
