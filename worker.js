...
async scheduled(ev,A,ctx){
  ...
  ctx.waitUntil((async()=>{
    try{
      ...
      /* 🚨 2026-09-08-e: BUGÜNKÜ KESİNTİNİN GERÇEK SEBEBİ — Cloudflare
         Workers KV ücretsiz planda GÜNDE 1000 yazma sınırı var. Aşağıdaki
         iki iş (tvkGecmisAdim, tvkCanli) dakikada 2'şer kez çalışıp her
         seferinde birkaç KV yazması yapıyordu — günde potansiyel olarak
         binlerce yazma, tek başına kotayı bitirebilecek kadar. Bugünkü
         "KV put() limit exceeded" hatası tam olarak buydu (mbTfTeshisUret
         yazmaHata alanında görüldü). Kalıcı çözüm ya Cloudflare Workers
         Paid plana ($5/ay, KV sınırı çok yüksek) geçmek ya da yazma
         sıklığını azaltmak — burada ikincisi yapıldı: artık her dakika
         değil, tvkCanli 3 dakikada bir, tvkGecmisAdim 10 dakikada bir
         (ve ikisi de artık dakikada 2 kez değil 1 kez) çalışıyor. Bu,
         önceki günlük yazma hacmini ~10-15 kata kadar azaltıyor. Yukarıdaki
         mbDilim/mbAlarm (başka, önceden var olan bir özellik) kasıtlı
         olarak dokunulmadan bırakıldı. */
      const dk=new Date(ev.scheduledTime||Date.now()).getMinutes();
      if(dk%3===0)
      ...
      if(dk%10===0){
        /* 🕰 Tavan Kombi geçmiş tarama artık tarayıcı sekmesine bağımlı
           değil — sekme kapansa/telefon kilitlense bile burada ilerler.
           Sayfa açıksa oradaki polling de aynı işi paylaşır (kilit
           sayesinde çakışmazlar), sayfa kapalıysa bile iş durmaz. */
