...
function mbMotor(mumlar){
  ...
  /* ⚡ 2026-09-09: son 60 bardaki ORTALAMA volatilite — (yüksek-düşük)/kapanış*100
     her barın kendi salınımı, 60 barın ortalaması. Zaten çekilmiş aynı mum
     verisinden hesaplanıyor, EK Yahoo isteği yok. */
  const vBarlar=m.slice(-60);
  let vToplam=0,vSayisi=0;
  for(const b of vBarlar){
    if(isFinite(b.high)&&isFinite(b.low)&&isFinite(b.close)&&b.close>0){
      vToplam+=100*(b.high-b.low)/b.close;vSayisi++;
    }
  }
...
async function mbOlc(kod,tfKod,onbellek,hataTopla){
  ...
  if(!ham){
    ...
    ham=(r&&r.veri)||[];
    /* 🩺 2026-09-06: ham boşsa SEBEBİ (yfMumlar'ın query1/query2 hataları)
       eskiden hiçbir yere kaydedilmiyordu — mbDilimTara "0 hisse ölçüldü"
       deyip geçiyordu, teşhis ekranı bunu Cron Trigger sorunuymuş gibi
       gösteriyordu. Artık gerçek hata örneği hataTopla'ya ekleniyor. */
    if(!ham.length&&hataTopla&&r&&r.hatalar&&r.hatalar.length)hataTopla.push(kod+": "+r.hatalar[r.hatalar.length-1]);
...
function hpHesapla(mumlar){
  try{
    ...
    /* Kırılıma mesafe (%) — kullanıcı isteği: "POC/kırılıma yüzde X kalanlar".
...
/* ---------- 🎯 SİNYAL TAKİBİ — POC/Kırılım'dan SONRA kâr/zarar ----------
   Kullanıcı isteği: bir hisse "🚀 Kırılım" veya "📈 POC Üstü" olduğunda bu
   anı unutmuyoruz — giriş fiyatı + zamanı KV'ye kaydedilir, sonraki her
   ölçümde o giriş fiyatına göre kâr/zarar %'si hesaplanıp satıra eklenir
   (sinyalKarYuzde). Sinyal zayıf tarafa (📉 POC Altı / ⚠️ Taban Altı)
   döndüğünde kayıt KAPANIR ve sonucu (ne kadar kazandırdı/kaybettirdi)
   dilim başına son 40 kayıtlık bir geçmiş listesine taşınır — "zarar
   edenler olduysa durum nedir" sorusunun cevabı bu liste (hpGecmisAl).
   ...
   🐞 KÖK NEDEN — 3. tur, GERÇEK sebep (2026-09-19): 1. ve 2. turda
   sorun "state nerede önbelleklenir"e odaklandı, ama asıl kusur ondan
   önceydi: TÜM hisselerin/dilimlerin durumu TEK bir "hpSinyal" KV
   anahtarında, TEK bir JSON blobda tutuluyordu. Canlı toplu ölçüm
   evreni (400+ hisse) onlarca AYRI isteğe (chunk) bölüp her chunk'ı
   ayrı bir HTTP isteğiyle ölçtürüyor; her chunk bu AYNI bloba kendi
   GET+PUT'unu yapıyordu. Cloudflare KV yazmaları ANINDA/GLOBAL tutarlı
   DEĞİLDİR — bir chunk'ın PUT'u daha yayılmadan bir sonraki chunk'ın
   GET'i başlarsa, o chunk eksik/eski bir kopya üstünde çalışıp bunu
   TÜM BLOBUN YERİNE geri yazıyor, bir öncekinin az önce eklediği yeni
   girişleri SİLİYORDU. Piyasa hareketliyken onlarca hisse AYNI ANDA
   "güçlü" durumuna geçtiğinde (çok sık rastlanan bir durum) bu neredeyse
   HER TARAMADA oluyordu — her chunk bir öncekini eziyor, sonuç hep
   "az önce girdi, %0".
   ÇÖZÜM: paylaşılan TEK blob tamamen kaldırıldı. Artık HER HİSSE/DİLİM
   çifti KENDİ KV ANAHTARINDA tutuluyor (hpAktif:{tf}:{kod}). Farklı
   hisseler ASLA aynı anahtara yazmaz — chunk'lar birbirini artık
   ezemez. Kapanan sinyaller dilim başına ayrı bir "hpGecmis:{tf}"
   anahtarında toplanır; bu hâlâ paylaşılan küçük bir liste ama
   yazması sadece bir sinyal KAPANDIĞINDA olur (her taramada değil),
   dolayısıyla çakışma ihtimali çok düşük ve sonucu en kötü ihtimalle
   "bir kapanış kaydı bazen listeye düşmeyebilir" — asla "her şey
   sıfırlanır" değil. */
const HP_GUCLU=new Set(["kirilim","poc_ustu"]);
const HP_SINYAL_TTL=7*86400, HP_SINYAL_GECMIS_AZAMI=40;
function hpAktifAnahtar(tf,kod){return "hpAktif:"+tf+":"+kod}
async function hpGecmisEkle(A,tf,kayit){
  const anahtar="hpGecmis:"+tf;
  try{
    let liste=[];
    try{const h=await A.VERI.get(anahtar);if(h)liste=JSON.parse(h)||[]}catch(_){}
    if(!Array.isArray(liste))liste=[];
    liste.unshift(kayit);
    liste=liste.slice(0,HP_SINYAL_GECMIS_AZAMI);
    await A.VERI.put(anahtar,JSON.stringify(liste),{expirationTtl:HP_SINYAL_TTL});
  }catch(_){}
}
...
async function hpSinyalIsle(A,tf,kod,s){
  ...
  try{
    ...
    let giris=null;
    try{const h=await A.VERI.get(anahtar);if(h)giris=JSON.parse(h)}catch(_){}
    const guclumu=HP_GUCLU.has(s.konum);
    if(guclumu){
      if(!giris){
        giris={fiyat:s.fiyat,ts:Date.now(),konum:s.konum};
        await A.VERI.put(anahtar,JSON.stringify(giris),{expirationTtl:HP_SINYAL_TTL});
      }
      ...
      const kar=Math.round(((s.fiyat-giris.fiyat)/giris.fiyat)*1000)/10;
      await hpGecmisEkle(A,tf,{kod:kod,girisFiyat:giris.fiyat,cikisFiyat:s.fiyat,
        karYuzde:kar,girisTs:giris.ts,cikisTs:Date.now(),enYuksekKonum:giris.konum});
...
const MB_BIRIKIM_TTL=21600,MB_YAZMA_ARALIK=6e5,MB_BAYAT_MS=864e5; /* 🚨 2026-09-09-c: 6 saatten 24 saate çıkarıldı — bkz. TVK_CANLI_ANLIK_AZAMI notu; bayat ölçüm SİLME eşiği, aşağıdaki "tamam mı" kontrolüyle karıştırılmasın */
...
/* 🩺 2026-09-06: son GERÇEK tarama denemesinin özeti (tf → {ts,denenen,
   basarili,ornekHata}). KV'ye YAZILMAZ, yalnız bu isolate belleğinde durur —
   amacı kalıcılık değil, "tara" butonuna basıldığında o anda ne olduğunu
   raporlamak. "0 hisse ölçülü" çıktığında bunun SEBEBİNİN taranmamış olmak
   mı (Cron Trigger/durdurulmuş/evren boş) yoksa TARANIP HEPSİNİN Yahoo'dan
   veri alamamış olması mı olduğunu ayırt etmek için — ikisi de dışarıdan
   aynı "0 hisse" görünüyordu ama sebepleri ve çözümleri tamamen farklı. */
let _mbSonTur={};
...
async function mbDilimTara(A,oncelikTf,azami,disImlec){
  /* 🩺 2026-09-06-b: bir önceki düzeltme yalnız TARAMA TAMAMLANDIĞINDA
     _mbSonTur'u dolduruyordu. Ama bu fonksiyon daha en baştaki
     kontrollerden birinde (tarama durdurulmuş / evren o an boş geldi vb.)
     erken çıkarsa hiçbir kayıt bırakmıyordu — ve mbTfTeshisUret birkaç
     satır sonra AYNI kontrolleri YENİDEN, o anki (belki artık farklı)
     durumla yaptığı için, o anlık/geçici arızayı hiç yakalayamıyordu.
     Şimdi HER çıkış noktası (erken dönüş dahil) kendi gerçek sebebini
     _mbSonTur'a yazıyor. */
  const anahtar=oncelikTf?mbTfNormal(oncelikTf):null;
  const kaydet=sebep=>{if(anahtar)_mbSonTur[anahtar]={ts:Date.now(),sebep:sebep,denenen:0,basarili:0,ornekHata:null}};
  if(!A||!A.VERI){kaydet("A/VERI eksik");return null;}
  if(!(await mbCalisiyorMu(A))){kaydet("tarama o an durdurulmuş görünüyordu");return null;}
  const evren=await mbEvren(A);               /* havuzun TAMAMI */
  if(!evren.length){kaydet("hisse listesi (evren) o an boş geldi");return null;}
  ...
  const t0=Date.now();let sira=0,islenen=0,hata=!1,basarili=0;const hataOrnek=[];
  const isci=async()=>{
    while(sira<kodlar.length){
      if(Date.now()-t0>MB_SURE_TAVAN_MS)return;
      const kod=kodlar[sira++];
      try{
        const s=await mbOlc(kod,tf,{},hataOrnek);
        if(s){s.ts=Date.now();delete s.tf;bir.sonuc[kod]=s;basarili++}   /* tf zaten anahtarda */
        else delete bir.sonuc[kod];
        islenen++;
      }catch(e){hata=!0;islenen++;if(hataOrnek.length<3)hataOrnek.push(kod+": istisna — "+((e&&e.message)||e))}
    }
  };
  try{await Promise.all(Array.from({length:Math.min(MB_ES,kodlar.length)},isci))}catch(_){hata=!0}
  /* 🩺 2026-09-06: bu turun GERÇEK sonucunu kaydet — bkz. _mbSonTur notu. */
  _mbSonTur[tf]={ts:Date.now(),denenen:islenen,basarili:basarili,ornekHata:hataOrnek[0]||null,
    sebep:islenen===0?"taranacak hisse dilimi boştu":(basarili===0?"hepsi denendi, hiçbiri başarılı olmadı":null)};
...
const TVK_ELEMAN=["dip","dip382","dip236","b1","b2","b3","b4","b5","b6","pivot","dunTavan"];
/* 🌹 2026-09-10: dunTavan'ın kendisi bir zaman dilimi (1SA/4SA) AYRIMI
   TAŞIMIYOR — tek bir günlük bayrak. Diğer 10 eleman gibi TVK_TF üzerinde
   genellenebilsin diye (tvkComboGecti/tvkGecenIdleriUret hiç değişmedi)
   snap.dunTavan["1SA"] ve snap.dunTavan["4SA"] AYNI değeri taşıyacak şekilde
   dolduruluyor (bkz. tvkGunSnapUret). Sonuç: raporda "dunTavan:1SA" ve
   "dunTavan:4SA" satırları HER ZAMAN birebir aynı n/isabet/oran'a sahip
   olacak — bu bir hata değil, elemanın doğası (tek dilim yok) böyle
   yansıyor. İkisi de görünse de aslında TEK bir sinyal. */
...
function tvkKombiler(){
  ...
  (function rec(i,secim){
    if(i===TVK_ELEMAN.length){
      if(secim.some(s=>s!==null))out.push(secim.slice());
      return;
    }
    rec(i+1,secim.concat([null]));
    for(const tf of TVK_TF)rec(i+1,secim.concat([tf]));
  })(0,[]);
  return out;
}
...
/* PİVOT KIRILIM'ın KISA(=1SA)/ORTA(=4SA) karşılığı — server tarafında
   canlı "kartlar" yerine zaten kalıcı olan "gecmis" arşivi kullanılır:
   BUGÜN o gün-kaydında İLK KEZ görülen kod@dilim = o gün kırılım olmuş
   demektir (mbPivotHaritasiS'in canlı halinin geçmişe dönük eşdeğeri). */
async function tvkPivotBugunSeti(A){
  const out={};
  try{
    const gp=await y(A);
    const d2=new Date(Date.now()+108e5),i=d2.toISOString().slice(0,10);
    const gun=gp.gunler&&gp.gunler[i];if(!gun)return out;
...
async function mbTfTeshisUret(A){
  ...
  /* 🔧 2026-09-03-i: teşhis eskiden yalnız "n hisse ölçülü / son yazım"
     ...
     /tara, /taraTumu her çağrıldığında kendi kendine dolduruyor olsa
     bile mesaj hâlâ eski öneriyi veriyordu, gerçek sebep başka bir şey
     olsa da (bot DURDURULMUŞ veya evren/hisse listesi BOŞ) kullanıcı
     yanlış yere bakıyordu. Artık buradan gerçek durum da okunup mesaja
     ekleniyor: calisiyor=false ise doğrudan "tarama durdurulmuş" denir,
  ...
  for(const t of TVK_TF){
    ...
    out.push({tf:t,n:n,yasDk:(b&&b.ts)?Math.round((Date.now()-b.ts)/6e4):null,parca:parcaSayisi,sonTur:_mbSonTur[t]||null});
  }
  return out;
}
async function tvkGunSnapUret(A){
  ...
  const dunTavanHaritasi=await tvkDunTavanOku(A);       /* 🌹 2026-09-10 */
  const kodlar=new Set();
  TVK_TF.forEach(t=>Object.keys(tfVeri[t]).forEach(k=>kodlar.add(k)));
  const sonuc={};
  kodlar.forEach(kod=>{
    const s={dip:{},dip382:{},dip236:{},b1:{},b2:{},b3:{},b4:{},b5:{},b6:{},fiyat:{},zaman:{},volatilite:{}};
    for(const t of TVK_TF){
      ...
      s.volatilite[t]=(x&&isFinite(x.volatilite60))?x.volatilite60:null;
    }
    ...
    const dt=!!dunTavanHaritasi[kod];               /* 🌹 2026-09-10 — tek bayrak, iki dilime aynen kopyalanır */
    s.dunTavan={"1SA":dt,"4SA":dt};
    sonuc[kod]=s;
  });
  return sonuc;
}
/* 🚨 2026-09-03-g · KÖKTEN DÜZELTME — "1SA: 0 hisse ölçülü (son yazım: hiç
   yazılmamış)" / Tavan Kombi canlı tarama hep "0 hisse tarandı" veriyordu.
   SEBEP: tvkGunSnapUret (yukarıda) TVK_TF (1SA/4SA) havuzunu YALNIZ OKUYOR,
   kendisi HİÇBİR ZAMAN bir tarama BAŞLATMIYORDU. Bu iki dilim başka hiçbir
   ekranda (MAL/DİP/AB/Bölge/Enerji alarm modülü ya da KISA/ORTA görünümü)
   o anda aktif olarak istenmiyorsa, havuzu doldurmanın TEK yolu ya
   Cloudflare Cron Trigger'ın (Settings → Triggers → Cron Triggers,
   scheduled() üstündeki kuruluma bak) çalışıyor olmasıydı ya da tesadüfen
   birinin o iki dilimi başka bir ekrandan tetiklemesiydi — hiçbiri
   olmayınca havuz SONSUZA DEK boş kalıyordu (teşhis mesajındaki "hiç
   yazılmamış" tam olarak bunu gösteriyor). ÇÖZÜM: Tavan Kombi'nin canlı
   uçları artık mbModulTara'daki "en bayat/en boş dilimi bul, bir adım
   ilerlet" mantığının TVK_TF'e daraltılmış halini KENDİLERİ çalıştırıyor —
   böylece özellik Cron Trigger kurulu olsun olmasın, sekmeyi her açan/
   "tara" diyen kullanıcıyla birlikte kendi kendini doldurur (MB_ANLIK_AZAMI
   ile sınırlı — tek istekte aşırı Yahoo çağrısı yapılmaz, birkaç açılışta
   havuz dolar).
   🚨 2026-09-09-c · KÖKTEN DÜZELTME — canlı havuz 443/444'e ulaştıktan
   SONRA tekrar 36'ya kadar KÜÇÜLDÜĞÜ görüldü. SEBEP: mbDilimTara'nın
   "bayat ölçümleri at" temizliği (6 saatten eski kaydı siler) — KV yazma
   kotasını korumak için tarama sıklığı yavaşlatılınca (2026-09-08-e), tam
   bir tur artık 6 saatten UZUN sürebiliyor: yeni hisseler eklenirken eski
   hisselerin süresi 6 saati geçip siliniyor, havuz asla 444'e tam
   ulaşamadan küçük kalıyor. Çözüm — MB_ANLIK_AZAMI (mbModulTara'nın da
   paylaştığı ortak sabit) yerine Tavan Kombi'ye ÖZEL, daha büyük bir
   parti boyutu (TVK_CANLI_ANLIK_AZAMI): her çağrıda daha çok hisse
   işleniyor, KV YAZMA SAYISI artmıyor (hâlâ tek PUT), tam tur çok daha
   hızlı bitiyor. Ayrıca MB_BAYAT_MS 6 saatten 24 saate çıkarıldı (bkz.
   tanım satırı) — artık yavaş cadence'e daha toleranslı. */
const TVK_CANLI_ANLIK_AZAMI=30;
async function tvkCanliDoldur(A){
  try{
    ...
    if(hedef)await mbDilimTara(A,hedef,TVK_CANLI_ANLIK_AZAMI).catch(()=>{});
  }catch(_){}
}
...
function tvkFiyatSec(kombo,s){
  ...
  for(let i=0;i<TVK_ELEMAN.length;i++){
    ...
    if(z&&(!en||z>en.zaman))en={fiyat:s.fiyat[tf],zaman:z,tf:tf,volatilite:s.volatilite&&s.volatilite[tf]!=null?s.volatilite[tf]:null};
  }
  return en;
}
/* 2.7'nin üstü "yüksek volatilite" sayılır — eşleşen hisse listesinde
   işaretlemek (client) ve /tavankombi Telegram özetinde süzmek için. */
const TVK_VOLATILITE_ESIK=2.7;
...
/* 🔧 2026-09-03-e: KV anahtarı "tvkArsiv" → "tvkArsiv2" olarak DEĞİŞTİRİLDİ.
   ...
   döner.
   🔧 2026-09-08: "tvkArsiv2" → "tvkArsiv3" — kalıcı (gün,hisse) çift takibi
   (ciftTakip) devreye girdi, artık turlar arası ASLA sıfırlanmayacak. Ama
   geçişte, ciftTakip bomboş başladığı için önceki (v2 dönemindeki) turlarda
   zaten sayılmış çiftler "yeni" sanılıp bir kez daha eklenebilirdi (çift
...
async function tvkOncekiFiyatYaz(A,v){if(A.VERI)try{await A.VERI.put("tvkOncekiFiyat",JSON.stringify(v))}catch(_){}}
/* 🌹 2026-09-10: "DÜN tavan yaptı mı" — kod bazında binary bayrak (son13Tavan
   gibi frekans değil, tam olarak DÜNKÜ gün için tavan olup olmadığı).
   tvkGunSonuIsle her gün sonunda o günün tavan sonucuyla ÜZERİNE YAZAR —
   birikmez, her zaman "en son işlenen gün" demektir. TVK_ELEMAN'a girip
   kombinasyonlarda test edilebilsin diye eklendi (bkz. WORKER_SURUM notu). */
async function tvkDunTavanOku(A){
  if(!A.VERI)return{};
  try{const j=await A.VERI.get("tvkDunTavan");if(j)return JSON.parse(j)}catch(_){}
  return{};
}
async function tvkDunTavanYaz(A,v){if(A.VERI)try{await A.VERI.put("tvkDunTavan",JSON.stringify(v))}catch(_){}}

/* Gün sonu — günde YALNIZ BİR KEZ çalışır (18:00 TRT sonrası ilk /push).
   e = /push gövdesi (e.sozluk = tüm evrenin o anki fiyatı). Kilit dışarıda
   (kilitli(...)) alınır, burada yalnız tvkSonGun bayrağı ile tekrar korunur. */
async function tvkGunSonuIsle(A,e){
  if(!A.VERI||!e||!e.sozluk)return;
  const d2=new Date(Date.now()+108e5),i=d2.toISOString().slice(0,10),saat=d2.getUTCHours();
  if(saat<18)return;                     /* kapanışa yakın bekle */
  let bayrak=null;
  try{bayrak=await A.VERI.get("tvkSonGun")}catch(_){}
  if(bayrak===i)return;                  /* bugün zaten işlendi */
  const yeniFiyat={};
  for(const kod of Object.keys(e.sozluk)){
    const z=e.sozluk[kod];if(z&&z.f>0)yeniFiyat[kod]=Number(z.f);
  }
  const oncekiFiyat=await tvkOncekiFiyatOku(A);
  const snap=await tvkGunSnapUret(A);
  const arsiv=await tvkArsivOku(A);
  arsiv.sayac=arsiv.sayac||{};
  let tavanSayisi=0,karsilastirma=0;
  const yeniDunTavan={};                         /* 🌹 bugünün sonucu → yarının "dünkü tavan" bayrağı */
  for(const kod of Object.keys(yeniFiyat)){
    const eski=oncekiFiyat[kod];
    if(!(eski>0))continue;                       /* ilk gün — kıyas yok */
    karsilastirma++;
    const degisim=100*(yeniFiyat[kod]/eski-1);
    const tavan=degisim>=TVK_ESIK;
    yeniDunTavan[kod]=tavan;
  ...
  arsiv.gun=(arsiv.gun||0)+1;
  arsiv.tavanToplam=(arsiv.tavanToplam||0)+tavanSayisi;
  arsiv.karsilastirmaToplam=(arsiv.karsilastirmaToplam||0)+karsilastirma;
  arsiv.sonGun=i;
  await tvkArsivYaz(A,arsiv);
  await tvkOncekiFiyatYaz(A,yeniFiyat);
  await tvkDunTavanYaz(A,yeniDunTavan);
  try{await A.VERI.put("tvkSonGun",i)}catch(_){}
}

function tvkRaporUret(arsiv){
  const satirlar=Object.keys(arsiv.sayac||{}).map(id=>{
    const s=arsiv.sayac[id];
    return{id:id,n:s.n,isabet:s.isabet,
...
/* 🔬 2026-09-10: Kombinasyondaki ELEMAN SAYISI (id'deki "+" adedi) ile
   isabet oranı arasındaki ilişki — 571 sisteminde görülen "çoklu uyum
   arttıkça sonuç kötüleşiyor" paradoksunun Tavan Kombi'de de geçerli olup
   olmadığını test eder. YENİ VERİ TOPLAMAZ — tvkGunSonuIsle zaten her
   geçen alt-kombinasyonu (tvkGecenIdleriUret = TÜM alt kümeler) tek tek
   sayıyordu, burada sadece mevcut arsiv.sayac'ı uzunluğa göre gruplayıp
   topluyoruz. Bir stok-gün, aynı gün içinde birden çok uzunluktaki
   kombinasyona (o gün TRUE olan elemanların her alt kümesine) katkı
   yapabildiği için satırlar birbirini dışlamaz — amaç "tam olarak N
   eleman true olan gün sayısı" değil, "N elemanlı bir şart arandığında
   ortalama isabet oranı ne oluyor" sorusuna cevap vermek; 571'deki
   karşılaştırma da aynı mantıkla yapılmıştı. */
function tvkUzunlukAnalizi(arsiv){
  const sepet={};
  const sayac=arsiv.sayac||{};
  for(const id in sayac){
    const s=sayac[id],uz=id.split("+").length;
    const b=sepet[uz]=sepet[uz]||{n:0,isabet:0,kombiSayisi:0};
    b.n+=s.n;b.isabet+=s.isabet;b.kombiSayisi++;
  }
  const taban=arsiv.karsilastirmaToplam?100*(arsiv.tavanToplam||0)/arsiv.karsilastirmaToplam:0;
  const satirlar=Object.keys(sepet).map(uz=>{
    const b=sepet[uz];
    return{uzunluk:Number(uz),n:b.n,isabet:b.isabet,oran:b.n?100*b.isabet/b.n:0,
      kombiSayisi:b.kombiSayisi,azOrnek:b.n<TVK_ASGARI};
  }).sort((a,b)=>a.uzunluk-b.uzunluk);
  return{ok:!0,gun:arsiv.gun||0,taban:taban,karsilastirmaToplam:arsiv.karsilastirmaToplam||0,
    asgari:TVK_ASGARI,satirlar:satirlar};
}
...
async function tvkGecmisIsYaz(A,job){
  if(!A.VERI)return;
  /* 🚨 2026-09-08: eskiden burada {expirationTtl:86400} vardı — iş 24
     saat içinde bitmezse kayıt KV'den kendiliğinden SİLİNİYORDU, sistem
     "iş hiç yok" sanıp SIFIRDAN yeni bir tam tur başlatıyordu, bu da
     birikmiş tvkArsiv2 istatistiklerinin silinmesine yol açıyordu
     (bkz. "yeni tam tur başlarken tvkArsiv2 sıfırlanır" mantığı). İş
...
/* 🔁 2026-09-08: KALICI, HİÇ SIFIRLANMAYAN (gün,hisse) çift takibi.
   ...
   pencere yeniden hesaplanıyordu (çift saymayı önlemenin TEK yolu buydu).
   Artık her (gün|kod) çifti bir kez işlenip buraya kaydediliyor; bir
   sonraki tur (otomatik ya da elle) AYNI çifte rastlarsa atlanıyor —
   yani her tur yalnızca GERÇEKTEN yeni olan günleri/hisseleri arsive
   ekliyor, tvkArsiv2 hiçbir zaman sıfırlanmıyor, sürekli büyüyor.
   ⚠️ Boyut notu: ~444 hisse × ~95-180 gün ≈ ilk turda birkaç yüz bin
   anahtar (birkaç MB); sonraki turlarda günde yalnız ~444 yeni anahtar
   eklenir. KV tek değer sınırı 25MB — yıllarca rahat yeter, ama uzun
...
/* 🔧 2026-09-08-g: hisseSayac için AYRI bir (gün|kod) takibi — bkz.
   ...
   boş başlıyor (bir kerelik geriye dönük doldurma için), ciftTakip'ten
   TAMAMEN bağımsız. */
...
function tvkGecmisSayacYeni(){return{sayac:{},tavanToplam:0,karsilastirmaToplam:0,gunSet:{},hisseSayac:{},hisseSon13:{}}}
...
async function tvkGecmisHisseTara(A,kod,gp,yerel,ciftTakip,hisseCiftTakip){
  ...
  /* 🌹 2026-09-09: "son 13 barda kaç kez tavan yapmış" — geriye dönük
     tahmin (yarının kapanışı) DEĞİL, o günün KENDİ kapanışının bir önceki
     güne göre ≥%7 olup olmadığı. Bu bir BİRİKİM değil, o an geçerli SON
     DURUM — her tarandığında baştan hesaplanıp ÜZERİNE YAZILIYOR
     (tvkGecmisBirlestir'de toplama değil, replace). */
  let son13=0;
  for(let i=Math.max(1,g1.length-13);i<g1.length;i++){
    const onceki=g1[i-1],simdi=g1[i];
    if(onceki&&onceki.close>0&&simdi&&simdi.close>0&&100*(simdi.close/onceki.close-1)>=TVK_ESIK)son13++;
  }
  yerel.hisseSon13=yerel.hisseSon13||{};
  yerel.hisseSon13[kod]=son13;
  let p1=-1,p4=-1;
  for(let i=0;i<g1.length-1;i++){
    ...
    const ciftId=gunStr+"|"+kod;
    /* 🔧 2026-09-08-g: comboSayildi/hisseSayildi AYRI takip ediliyor.
       Sebep: hisseSayac (hissenin kendi tavan alışkanlığı) ciftTakip'ten
       SONRA eklendi — round 1 zaten ciftTakip'i doldurup bitmişti, bu
       yüzden hisseSayac hiç dolamadı (her şey "zaten sayıldı" sanılıp
       atlanıyordu). Ayrı bir izleyici (hisseCiftTakip, boş başlıyor)
       sayesinde combo istatistikleri ÇİFT SAYILMADAN (ciftTakip hâlâ
       koruyor), hisseSayac bir kerelik geriye dönük olarak dolduruluyor. */
    const comboSayildi=!!(ciftTakip&&ciftTakip[ciftId]);
    const hisseSayildi=!!(hisseCiftTakip&&hisseCiftTakip[ciftId]);
    if(comboSayildi&&hisseSayildi)continue;
    ...
    if(!hisseSayildi){
      /* 🎯 2026-09-08: hissenin KENDİ tavan alışkanlığı — kombinasyondan
         bağımsız, "bu hisse toplam N günün kaçında tavan yapmış" sayacı.
         Amaç: bir kombinasyon şu an THYAO'yu da SASA'yı da eşleştirse bile,
         THYAO'nun tavan yapma alışkanlığı neredeyse hiç yokken SASA'nınki
         yüksek olabilir — kullanıcı bunu görüp seçebilsin. */
      const hs=yerel.hisseSayac[kod]=yerel.hisseSayac[kod]||{n:0,isabet:0};
      hs.n++;if(tavan)hs.isabet++;
      if(hisseCiftTakip)hisseCiftTakip[ciftId]=1;
    }
  }
}
...
async function tvkGecmisBirlestir(A,yerel,gunYeniSayisi){
  ...
  arsiv.hisseSayac=arsiv.hisseSayac||{};
  for(const kod in yerel.hisseSayac){
    const s=yerel.hisseSayac[kod],hedef=arsiv.hisseSayac[kod]=arsiv.hisseSayac[kod]||{n:0,isabet:0};
    hedef.n+=s.n;hedef.isabet+=s.isabet;
  }
  /* 🌹 2026-09-09: hisseSon13 BİRİKİM değil — her taramada o hissenin en
     güncel "son 13 bar tavan sayısı"yla ÜZERİNE YAZILIYOR (toplanmıyor). */
  arsiv.hisseSon13=arsiv.hisseSon13||{};
  for(const kod in (yerel.hisseSon13||{}))arsiv.hisseSon13[kod]=yerel.hisseSon13[kod];
...
/* Bir hissenin KENDİ tavan alışkanlığını {n,isabet,oran} olarak döner —
   20'den az gözlem varsa null (henüz güvenilir değil, "az örnek" ile aynı eşik). */
function tvkHisseTavanOrani(arsiv,kod){
  const s=arsiv&&arsiv.hisseSayac&&arsiv.hisseSayac[kod];
  if(!s||s.n<20)return null;
  return{n:s.n,isabet:s.isabet,oran:Math.round(1e3*s.isabet/s.n)/10};
}
/* 🌹 son 13 barda kaç kez GERÇEKTEN tavan yapmış (o günün kendi kapanışı,
   yarının değil) — null/0 ise işaretlenmez. */
function tvkHisseSon13Tavan(arsiv,kod){
  const s=arsiv&&arsiv.hisseSon13&&arsiv.hisseSon13[kod];
...
/* Yeni bir tam tur kurar — kuyruğa TÜM evren yeniden alınır ama tvkArsiv2'ye
   VE ciftTakip'e DOKUNULMAZ (ikisi de kalıcı, turlar arası hiç sıfırlanmaz).
   ciftTakip sayesinde bu yeni tur, daha önce sayılmış (gün,hisse) çiftlerini
   otomatik atlayıp yalnızca GERÇEKTEN yeni olanları arşive ekler. */
async function tvkGecmisYeniTurBaslat(A,anahtar){
  ...
  const job={anahtar:anahtar||"",kuyruk:evren.slice(),toplam:evren.length,tamam:0,gunToplam:0,
    ...
    tur:1};
  const eski=await tvkGecmisIsOku(A);
  if(eski&&eski.tumGunSet)job.tumGunSet=eski.tumGunSet;   /* "kaç farklı gün görüldü" sayacı da kalıcı */
  if(eski&&eski.tur)job.tur=eski.tur+1;
  await tvkGecmisIsYaz(A,job);
  return job;
}
async function tvkGecmisAdimYap(A){
  let job=await tvkGecmisIsOku(A);
  /* 🔁 2026-09-08: iş hiç yoksa (ilk kurulum) YA DA önceki tur bitip
     üzerinden ≥20 dakika geçtiyse, kendiliğinden YENİ bir tur başlat —
     artık kimsenin sayfayı elle açmasına gerek yok, her gün otomatik
     tekrar taranır. tvkArsiv2/ciftTakip'e dokunulmaz, yalnız kuyruk
     yeniden dolar. */
  if(!job||(job.tamamlandi&&Date.now()-(job.tamamlandiTs||0)>=12e5))
    job=await tvkGecmisYeniTurBaslat(A,job&&job.anahtar);
  if(job.tamamlandi||!job.kuyruk.length)return job;
  const grup=job.kuyruk.splice(0,TVK_GECMIS_ADIM);
  const gp=await y(A);
  const ciftTakip=await tvkCiftTakipOku(A);
  const hisseCiftTakip=await tvkHisseCiftTakipOku(A);
  ...
  for(const kod of grup){
    try{await tvkGecmisHisseTara(A,kod,gp,job.yerel,ciftTakip,hisseCiftTakip)}
    catch(e){
  ...
  await tvkCiftTakipYaz(A,ciftTakip);
  await tvkHisseCiftTakipYaz(A,hisseCiftTakip);
  ...
  if(!job.kuyruk.length){job.tamamlandi=!0;job.tamamlandiTs=Date.now()}
  await tvkGecmisIsYaz(A,job);
  return job;
}
...
function ekranAdi(){
  ...
  if(sekme==="tavankombi")return"🎯 Tavan Kombi";
...
function ciz(){
  ...
  if(sekme==="tavankombi")return tavanKombiCiz();
...
function tvkTeshisMetni(liste){
  ...
  var parcalar=liste.map(function(d){
    ...
    /* 🩺 2026-09-06: "tara" butonuna basınca bu tf için GERÇEKTEN bir tur
       denendiyse, o turun kaç/kaç başarılı olduğunu da göster — "0 hisse
       ölçülü" yazısı artık hiç denenmemiş mi yoksa denenip hepsi
       başarısız mı olduğunu ayırt ediyor. */
    var tur=(d.sonTur&&d.sonTur.denenen)?(" [bu turda "+d.sonTur.basarili+"/"+d.sonTur.denenen+" başarılı]"):"";
    return d.tf+": "+d.n+" hisse ölçülü (son yazım: "+yas+")"+tur;
  });
  var bosVarMi=liste.some(function(d){return d.n===0||d.yasDk==null||d.yasDk>20});
  var not="";
  if(bosVarMi){
    /* 🩺 2026-09-06: bu turda GERÇEKTEN denenip hepsi başarısız olan bir tf
       var mı? Varsa bu en kesin ve en spesifik sebep — Cron Trigger hiç
       ilgisi yok, tarama zaten çalışıyor ama Yahoo'dan veri gelmiyor.
       yazmaHata/durdurulmuş/evren-boş kontrollerinden SONRA, ama genel
       "Cron Trigger'a bak" tahmininden ÖNCE gelir. */
    var basarisizTf=liste.filter(function(d){return d.sonTur&&(d.sonTur.sebep||(d.sonTur.denenen>0&&d.sonTur.basarili===0))});
    if(liste.yazmaHata){
      not=" ⚠️ KV YAZMA BAŞARISIZ: "+liste.yazmaHata+" — en olası sebep Cloudflare Workers KV ücretsiz plandaki günlük 1000 yazma kotasının dolmuş olması (cron her dakika yazıyor, gün içinde kolay dolar). Kota Türkiye saatiyle 03:00 civarı sıfırlanır, o zaman kendiliğinden düzelir. Kalıcı çözüm: Cloudflare Workers Paid plana ($5/ay) geçmek.";
    }else if(liste.calisiyor===!1){
    ...
    }else if(basarisizTf.length){
      not=" ⚠️ Cron Trigger sorunu DEĞİL — bu tıklamada gerçekten deneniyor: "+
        basarisizTf.map(function(d){return d.tf+" — "+(d.sonTur.sebep||(d.sonTur.denenen+" hisse denendi, hepsi başarısız"))+(d.sonTur.ornekHata?" (örnek: "+d.sonTur.ornekHata+")":"")}).join(", ")+
        ". Birkaç dakika bekleyip tekrar 'şimdi tara'ya bas; sorun sürekli aynı mesajla devam ediyorsa bu satırı ilet.";
    }else{
...
function tvkFiyatEtiket(o){
  var kod=E(o.kod);
  /* ⚡ 2026-09-09: son 60 bardaki ortalama volatilite 2.7'nin üstündeyse
     isim yanına küçük bir işaret. Amaç: aynı kombinasyonu karşılasa bile
     "sakin" bir hisse ile "oynak" bir hisseyi ayırt edebilmek. */
  if(o&&o.volatilite!=null&&o.volatilite>2.7)kod='⚡'+kod;
  /* 🌹 2026-09-09: son 13 GÜNLÜK barda kaç kez GERÇEKTEN tavan yapmış (o
     günün kendi kapanışı bir önceki güne göre ≥%7) — geriye dönük tahmin
     değil, somut yakın geçmiş. 0 ise hiç işaret yok (detay kirliliği
     olmasın diye), sadece gerçekten olmuşsa sayı görünür. */
  if(o&&o.son13Tavan)kod+='🌹'+o.son13Tavan;
  /* 🎯 2026-09-08: hissenin KENDİ tavan alışkanlığı — kombinasyon eşleşse
     bile THYAO gibi neredeyse hiç tavan yapmayan bir hisseyle SASA gibi
     sık tavan yapan bir hisseyi ayırt edebilmek için. 20'den az gözlem
     varsa (hisseTavan null) hiçbir şey eklenmiyor — "az örnek" ile aynı
     eşik, yanıltıcı erken bir yüzde göstermemek için. */
  var tvHtml="";
  if(o&&o.hisseTavan){
    var oran=o.hisseTavan.oran;
    var renk=oran>=8?"#3fb950":(oran>=4?"#d29922":"#8b949e");
    tvHtml='<br><span style="color:'+renk+';font-weight:normal;font-size:11px">tavan alışkanlığı: %'+oran+' ('+o.hisseTavan.n+' gün)</span>';
  }
  if(!o||!o.zaman)return kod+tvHtml;
  ...
  return kod+'<br><span style="color:#8b949e;font-weight:normal;font-size:11px">'+
    (o.fiyat!=null?o.fiyat.toFixed(2)+' TL · ':'')+gg+'.'+aa+' '+ss+':'+dd+'</span>'+tvHtml;
}
...
function tavanKombiGoster(v){
  ...
  v.satirlar.slice(0,GOSTERILEN).forEach(function(r,i){
    ...
    h+='<div class="btGun">'+
       ...
       '<span class="btN">'+r.n+' gözlem</span>'+
       '<span class="'+(ustu?"ye":"kr")+'"><b>%'+r.oran.toFixed(1)+'</b></span></div>'+
...
function hpSinyalSatiri(x){
  ...
  var pozitif=x.sinyalKarYuzde>=0;
...
function hpGecmisSatir(x){
  ...
  var renk=pozitif?"var(--yes)":"var(--kir)";
  var kbGiris=HP_KONUM_BILGI[x.enYuksekKonum];
  var girisRozet=kbGiris?' <span class="rozet" style="background:'+kbGiris.renk+';color:#0e1116">'+kbGiris.ad+'</span>':"";
  var saatFarki=Math.max(0,Math.round((x.cikisTs-x.girisTs)/36e5));
  var sureTxt=saatFarki<24?(saatFarki+" saat"):(Math.round(saatFarki/24)+" gün");
  return '<div class="satir" style="border-left-color:'+renk+';align-items:flex-start">'+
    '<div class="sol"><div class="kod">'+E(x.kod)+girisRozet+'</div>'+
    '<div class="altbilgi">giriş <b>'+x.girisFiyat+'</b> → çıkış <b>'+x.cikisFiyat+'</b> · '+sureTxt+' sürdü</div></div>'+
...
if("/tavankombi/gecmis"===$.pathname){
  ...
  /* 🔁 2026-09-08 · ARTIK yeni tur otomatik: her tur bitince 20 dakika
     sonra cron (tvkGecmisAdimYap içinde) kendiliğinden yeni bir tur
     başlatıyor, ciftTakip sayesinde bu güvenli (yalnız gerçekten yeni
     (gün,hisse) çiftlerini ekliyor, tvkArsiv2'yi ASLA sıfırlamıyor).
     Bu sayfa artık kendiliğinden yıkıp-yeniden-kurmuyor — yalnız hiç iş
     yoksa (ilk kurulum) bir tane başlatıyor; tamamlanmış bir iş varsa
     onun son durumunu gösterip bir sonraki cron turunu bekliyor. */
  const job=mevcutIs||await tvkGecmisYeniTurBaslat(A,anahtar);
  return new Response(tvkGecmisIlerlemeHTML(anahtar||job.anahtar),{headers:{"content-type":"text/html; charset=utf-8"}});
}
...
/* 📊 Geçmiş Sinyaller — bir hisse 🚀 Kırılım/📈 POC Üstü'nden zayıf tarafa
   döndüğünde kapanan kayıtlar. "Zarar edenler olduysa durum nedir" cevabı. */
if(gov&&gov.is==="gecmis"){
  const tf=HP_TF_LISTE.indexOf(gov.tf)>=0?gov.tf:"1G";
...
if("/api/tavankombi/tara"===$.pathname){
  ...
  const arsiv5=await tvkArsivOku(A);
  const kodlar5=Object.keys(snap5).filter(kod=>tvkComboGecti(kombo,snap5[kod])).sort()
    .map(kod=>Object.assign({kod:kod,hisseTavan:tvkHisseTavanOrani(arsiv5,kod),son13Tavan:tvkHisseSon13Tavan(arsiv5,kod)},tvkFiyatSec(kombo,snap5[kod])));
...
if("/api/tavankombi/taraTumu"===$.pathname){
  ...
  const arsiv6=await tvkArsivOku(A);
  ...
  for(const id6 of idler){
    ...
    sonuclar6[id6]=kombo6?{ok:!0,kodlar:kodTum6.filter(kod=>tvkComboGecti(kombo6,snap6[kod])).sort()
      .map(kod=>Object.assign({kod:kod,hisseTavan:tvkHisseTavanOrani(arsiv6,kod),son13Tavan:tvkHisseSon13Tavan(arsiv6,kod)},tvkFiyatSec(kombo6,snap6[kod])))}:{ok:!1,mesaj:"geçersiz kombinasyon"};
  }
...
text:(s2.ok?"✅ ":"⚠️ ")+E2(s2.mesaj),parse_mode:"HTML",reply_markup:u(t.from.id)})})()),new Response("ok")
;if(i&&n.startsWith("/baslat"))return q.waitUntil((async()=>{
  if(!d(t.from.id)){await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,
    text:"Bu komut yalnızca yöneticiye açık."});return}
  await mbDurdurAyarla(A,!1);
  await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,text:"✅ Tarama başlatıldı (artık DURDURULMUŞ değil).",reply_markup:u(t.from.id)})})()),new Response("ok")
;if(i&&n.startsWith("/durdur"))return q.waitUntil((async()=>{
  if(!d(t.from.id)){await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,
    text:"Bu komut yalnızca yöneticiye açık."});return}
...
;if(i&&n.startsWith("/tavankombi"))return q.waitUntil((async()=>{
  /* 📋 2026-09-09: Tavan Kombi'nin "en iyi kombinasyonlar + şu an eşleşen
     ...
     gerek kalmadan. Yalnız yönetici (tvkCanliDoldur + arşiv okuma + canlı
     tarama tetikliyor, hafif bir iş değil). ⚡ işareti = son 60 bardaki
     ortalama volatilitesi 2.7'nin üstünde olan hisse. */
  if(!d(t.from.id)){await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,
    text:"Bu komut yalnızca yöneticiye açık."});return}
  await tvkCanliDoldur(A);
  const arsivT=await tvkArsivOku(A);
  const raporT=tvkRaporUret(arsivT);
  const snapT=await tvkGunSnapUret(A);
  const satirlarT=raporT.satirlar.filter(r=>!r.azOrnek);
  let mT="🎯 <b>Tavan Kombi</b> — en iyi kombinasyonlar ("+raporT.gun+" gün, "+raporT.karsilastirmaToplam+" gözlem)\n"+
    "⚡ = son 60 barda ort. volatilite > %"+TVK_VOLATILITE_ESIK+" · 🌹N = son 13 günlük barda N kez gerçek tavan\n\n";
  let gosterilenT=0;
  for(const rowT of satirlarT){
    if(gosterilenT>=12)break;                        /* Telegram 4096 karakter sınırı — bkz. -e-d dersi */
    const komboT=tvkComboFromId(rowT.id);
    if(!komboT)continue;
    const kodlarT=Object.keys(snapT).filter(kod=>tvkComboGecti(komboT,snapT[kod]))
      .map(kod=>Object.assign({kod:kod,son13Tavan:tvkHisseSon13Tavan(arsivT,kod)},tvkFiyatSec(komboT,snapT[kod])))
      .sort((a,b2)=>(b2.volatilite||0)-(a.volatilite||0));
    if(!kodlarT.length)continue;                      /* şu an kimse karşılamıyorsa atla */
    gosterilenT++;
    mT+="<b>"+E2(rowT.id)+"</b> — %"+(Math.round(rowT.oran*10)/10)+" ("+rowT.n+" gözlem)\n";
    const gosterT=kodlarT.slice(0,8);
    mT+=gosterT.map(k=>(k.volatilite!=null&&k.volatilite>TVK_VOLATILITE_ESIK?"⚡":"")+E2(k.kod)+(k.son13Tavan?"🌹"+k.son13Tavan:"")+
      (k.fiyat!=null?" "+k.fiyat.toFixed(2)+"TL":"")).join(", ");
    if(kodlarT.length>gosterT.length)mT+=" +"+(kodlarT.length-gosterT.length)+" tane daha";
    mT+="\n\n";
  }
...
;if(i&&n.startsWith("/tavanuzunluk"))return q.waitUntil((async()=>{
  ...
  if(!d(t.from.id)){await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,
    text:"Bu komut yalnızca yöneticiye açık."});return}
  const arsivU=await tvkArsivOku(A);
  const rU=tvkUzunlukAnalizi(arsivU);
  if(!rU.satirlar.length){await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,
    text:"Henüz arşivde veri yok — en az bir gün sonu (18:00 TRT sonrası) geçmesi gerekiyor.",reply_markup:u(t.from.id)});return}
  let mU="🔬 <b>Eleman sayısı ↔ isabet oranı</b> ("+rU.gun+" gün, "+rU.karsilastirmaToplam+" gözlem)\n"+
    "Taban (filtresiz tavan oranı): %"+(Math.round(rU.taban*10)/10)+"\n"+
    "⚠️ n&lt;"+rU.asgari+" = az örnek, dikkatli oku\n\n";
  for(const rw of rU.satirlar){
    mU+=(rw.azOrnek?"⚠️ ":"")+"<b>"+rw.uzunluk+" eleman</b>: %"+(Math.round(rw.oran*10)/10)+
      " isabet ("+rw.n+" gözlem, "+rw.kombiSayisi+" farklı kombinasyon)\n";
  }
...
;if(i&&n.startsWith("/tavantani"))return q.waitUntil((async()=>{
  /* 🔬 2026-09-06-g: /surum'daki gibi mini app'e (MINIAPP client script'ine)
     ...
     ham teşhis verisini gönderiyor. Amaç: -e'de mini app'i UI üzerinden
     bozan riski tekrar almadan, aynı ham veriyi (calisiyor/evrenSayisi/
     yazmaHata/sonTur) güvenli bir yoldan görebilmek. */
  if(!d(t.from.id)){await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,
    text:"Bu komut yalnızca yöneticiye açık."});return}
  await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,text:"🔬 Gerçek bir tarama denemesi yapılıyor…"});
  let m="🔬 <b>Tavan Kombi — ham teşhis</b>\n\n";
  try{
    await tvkCanliDoldur(A);           /* gerçek bir Yahoo denemesi tetikle */
    const liste=await mbTfTeshisUret(A);
    m+="çalışıyor: <code>"+E2(String(liste.calisiyor))+"</code>\n";
    m+="evren sayısı: <code>"+E2(String(liste.evrenSayisi))+"</code>\n";
    m+="KV yazma hatası: <code>"+E2(liste.yazmaHata||"yok")+"</code>\n\n";
    for(const d2 of liste){
      m+="<b>"+E2(d2.tf)+"</b>: "+d2.n+" hisse, son yazım "+(d2.yasDk==null?"hiç":d2.yasDk+" dk önce")+"\n";
      m+="<code>"+E2(d2.sonTur?JSON.stringify(d2.sonTur):"sonTur: null (bu turda hiç denenmedi)")+"</code>\n\n";
    }
    /* 🎯 2026-09-08-d: hisse başına tavan alışkanlığı verisi doluyor mu? */
    const arsivD=await tvkArsivOku(A);
    const hs=arsivD.hisseSayac||{};
    const kodlarD=Object.keys(hs);
    m+="hisseSayac dolu mu: <code>"+kodlarD.length+" hisse</code>\n";
    if(kodlarD.length){
      const ornekD=kodlarD.slice(0,5).map(k=>k+":"+JSON.stringify(hs[k])).join(" | ");
      m+="<code>"+E2(ornekD)+"</code>\n\n";
    }
    /* 🔧 2026-09-08-f: geçmiş tarama işi (tvkGecmisAdimYap) gerçekten
       ilerliyor mu? hisseSayac boşsa en olası sebep bu işin hiç
       çalışmamış/takılmış olması. */
    const isD=await tvkGecmisIsOku(A);
    m+="geçmiş tarama işi: <code>"+E2(isD?JSON.stringify({tamam:isD.tamam,toplam:isD.toplam,tamamlandi:isD.tamamlandi,tur:isD.tur,guncelleme:isD.guncelleme?new Date(isD.guncelleme).toISOString():null,sonHatalar:(isD.sonHatalar||[]).slice(-2)}):"iş yok (null)")+"</code>\n\n";
    /* 🔧 2026-09-08-h: neden ilerlemiyor sorusuna KESİN cevap için,
       tvkGecmisAdimYap'ı ŞİMDİ, burada, doğrudan çalıştırıp
       fırlatırsa hatayı ANINDA yakalıyoruz — cron'u/hataYaz kaydını
       beklemeye gerek yok. */
    try{
      const yeniIs=await tvkGecmisAdimYap(A);
      m+="elle çalıştırma sonucu: <code>"+E2(JSON.stringify({tamam:yeniIs&&yeniIs.tamam,toplam:yeniIs&&yeniIs.toplam,tamamlandi:yeniIs&&yeniIs.tamamlandi,tur:yeniIs&&yeniIs.tur}))+"</code>";
    }catch(e2){
      m+="⚠️ tvkGecmisAdimYap FIRLATTI: <code>"+E2(String((e2&&e2.stack)||(e2&&e2.message)||e2).slice(0,500))+"</code>";
    }
  }catch(e){m+="Hata: "+E2(String((e&&e.message)||e))}
  await b(A.BOT_TOKEN,"sendMessage",{chat_id:t.chat.id,text:m,parse_mode:"HTML",reply_markup:u(t.from.id)})})()),new Response("ok")
...
try{
  ...
  m+="🖥 <b>Tarayıcı</b> (yumatu.html)\n<code>"+
     E2(String((L&&L.tarayiciSurum)||"— damga yok, ESKİ DOSYA —"))+"</code>\n\n";
  m+="🕐 Son tarama: "+(ts?ts.toLocaleString("tr-TR",{timeZone:"Europe/Istanbul"}):"—")+
     (yas!=null?" ("+yas+" dk önce)":"")+"\n";
  m+="📦 Beklenen: <code>"+E2(BEKLENEN_TARAYICI_SURUM)+"</code>\n\n";
  ...
  /* 🩺 2026-09-06-d: WORKER_SURUM her düzeltmede başına yeni not eklene ede
     10.000+ karaktere ulaştı — Telegram'ın sendMessage 4096 karakter sınırını
     aşınca istek sessizce reddediliyordu (hiç mesaj gelmiyordu, hata da
     görünmüyordu). Artık yalnız EN SON (en baştaki) girdi gösteriliyor. */
  const surumKisa=WORKER_SURUM.split(/ · (?=\d{4}-\d{2}-\d{2}-)/)[0];
  m+="\n\n⚙️ <b>Worker</b> (son not)\n<code>"+E2(surumKisa)+"</code>";
}catch(e){m+="Okunamadı."}
...
if("elletara"===r){
  ...
  return q.waitUntil((async()=>{const s2=await taramaTetikle(A);
    await b(A.BOT_TOKEN,"sendMessage",{chat_id:i,text:(s2.ok?"✅ ":"⚠️ ")+E2(s2.mesaj),
...
async scheduled(ev,A,ctx){
  ...
  ctx.waitUntil((async()=>{
    try{
      ...
      /* 🚨 2026-09-08-i · KÖKTEN DÜZELTME — dakika bazlı "dk%N===0" kapısı
         GERÇEKTE hiç tetiklenmiyordu: geçmiş tarama işi saatlerce 14:14'te
         donuk kaldı, /tavantani'den ELLE çağrılınca sorunsuz çalıştı — yani
         sorun tvkGecmisAdimYap'ta değil, bu dakika hesabındaydı (muhtemel
         sebep: ev.scheduledTime bu ortamda beklenenden farklı davranıyor
         ya da cron her tetiklemede TAM dakika sınırında çalışmıyor). Dakika
         aritmetiğine hiç güvenmeyen, kendi kendine yeten bir kapıya
