...
function tvkTeshisMetni(liste){
  ...
  if(bosVarMi){
    ...
    var basarisizTf=liste.filter(function(d){return d.sonTur&&d.sonTur.denenen>0&&d.sonTur.basarili===0});
    if(liste.yazmaHata){
      ...
      not=" ⚠️ Cron Trigger sorunu DEĞİL — tarama gerçekten deneniyor ama Yahoo'dan veri alınamıyor: "+
        basarisizTf.map(function(d){return d.tf+" ("+d.sonTur.denenen+" hisse denendi, "+d.sonTur.denenen+"/"+d.sonTur.denenen+" başarısız"+(d.sonTur.ornekHata?" — örnek: "+d.sonTur.ornekHata:"")+")"}).join(", ")+
        ". Muhtemel sebep: Yahoo Finance'in Cloudflare'in IP'lerini geçici olarak sınırlaması/engellemesi. Birkaç dakika bekleyip tekrar 'şimdi tara'ya bas; sorun sürekli aynı hatayla devam ediyorsa yukarıdaki örnek hata mesajını ilet.";
    }else{
