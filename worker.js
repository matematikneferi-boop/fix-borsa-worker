...
function tvkGecmisSayacYeni(){return{sayac:{},tavanToplam:0,karsilastirmaToplam:0,gunSet:{},hisseSayac:{}}}
...
if("/api/tavankombi/tara"===$.pathname){
  ...
  const kodlar5=Object.keys(snap5).filter(kod=>tvkComboGecti(kombo,snap5[kod])).sort()
    .map(kod=>Object.assign({kod:kod,hisseTavan:tvkHisseTavanOrani(arsiv5,kod)},tvkFiyatSec(kombo,snap5[kod])));
...
if("/api/tavankombi/taraTumu"===$.pathname){
  ...
  for(const id6 of idler){
    ...
    sonuclar6[id6]=kombo6?{ok:!0,kodlar:kodTum6.filter(kod=>tvkComboGecti(kombo6,snap6[kod])).sort()
      .map(kod=>Object.assign({kod:kod,hisseTavan:tvkHisseTavanOrani(arsiv6,kod)},tvkFiyatSec(kombo6,snap6[kod])))}:{ok:!1,mesaj:"geçersiz kombinasyon"};
  }
...
;if(i&&n.startsWith("/tavankombi"))return q.waitUntil((async()=>{
  ...
  let mT="🎯 <b>Tavan Kombi</b> — en iyi kombinasyonlar ("+raporT.gun+" gün, "+raporT.karsilastirmaToplam+" gözlem)\n"+
    "⚡ = son 60 barda ort. volatilite > %"+TVK_VOLATILITE_ESIK+"\n\n";
  let gosterilenT=0;
  for(const rowT of satirlarT){
    ...
    const kodlarT=Object.keys(snapT).filter(kod=>tvkComboGecti(komboT,snapT[kod]))
      .map(kod=>Object.assign({kod:kod},tvkFiyatSec(komboT,snapT[kod])))
    ...
    mT+=gosterT.map(k=>(k.volatilite!=null&&k.volatilite>TVK_VOLATILITE_ESIK?"⚡":"")+E2(k.kod)+
